...
(async () => {
  ...
  const apple = mergeByReviewId(
    previousRecords('myntra_app_store_last_1000.json').length ? previousRecords('myntra_app_store_last_1000.json') : previousRecords('myntra_app_store_last_500.json'),
    freshApple
  );
  const play = mergeByReviewId(
    previousRecords('myntra_google_play_last_1000.json').length ? previousRecords('myntra_google_play_last_1000.json') : previousRecords('myntra_google_play_last_500.json'),
    freshPlay
  );
  const recentApple = apple.filter(item => new Date(item.date) >= cutoff);
  const recentPlay = play.filter(item => new Date(item.date) >= cutoff);
  writeJson('myntra_app_store_last_1000.json', apple);
  writeJson('myntra_google_play_last_1000.json', play);
  writeJson('myntra_app_store_last_30_days.json', recentApple);
  writeJson('myntra_google_play_last_30_days.json', recentPlay);
  const report = {
