...
try {
  ...
  // Render LLM Executive Summary
  const execSummary = analysis.executive_summary 
    ? `<p style="font-size: 15px; color: #444; background-color: #fafbfc; border-left: 4px solid #ff3f6c; padding: 12px; border-radius: 4px; line-height: 1.6;">${analysis.executive_summary}</p>`
    : '<p>Weekly theme trends remain stable across the evaluated period.</p>';

  // Render LLM Emerging Themes
  let themesHtml = '';
  ...
  // Render LLM Recommended Actions (Non-Monetary Experiments)
  let actionsHtml = '';
  if (analysis.recommended_actions && analysis.recommended_actions.length > 0) {
    actionsHtml = '<ol style="margin: 0; padding-left: 20px;">' + 
      analysis.recommended_actions.map(a => `<li style="margin-bottom: 8px; font-weight: 500;"><span style="font-weight: normal; color: #333;">${a}</span></li>`).join('') + 
      '</ol>';
  } else {
    actionsHtml = '<p>No recommended actions generated this week.</p>';
  }
  ...
  const emailHtml = `
    <html>
      <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #333; line-height: 1.6; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #f0f0f0; border-radius: 8px;">
        ...
        <p>Hi PM Team,</p>
        <p>The weekly analysis of public Myntra feedback from App Stores, Reddit, Quora, and Shopping Communities is complete. Here are the core insights generated by the AI:</p>

        <h3 style="border-bottom: 1px solid #ddd; padding-bottom: 5px; color: #333; margin-top: 20px;">📝 Executive Summary</h3>
        ${execSummary}

        <h3 style="border-bottom: 1px solid #ddd; padding-bottom: 5px; color: #333; margin-top: 25px;">📢 Emerging Weekly Themes</h3>
        ${themesHtml}

        <h3 style="border-bottom: 1px solid #ddd; padding-bottom: 5px; color: #333; margin-top: 25px;">💡 Problem Hypotheses</h3>
        ${hypothesesHtml}

        <h3 style="border-bottom: 1px solid #ddd; padding-bottom: 5px; color: #333; margin-top: 25px;">🚀 Recommended Non-Monetary Actions</h3>
        ${actionsHtml}
  ...
  console.log('Successfully formatted HTML email report with LLM narratives.');
} catch (error) {
