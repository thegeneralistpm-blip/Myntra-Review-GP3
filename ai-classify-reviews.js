...
const systemPrompt = `You are classifying public Myntra customer feedback for a product-management research project on wishlist conversion. Return JSON only. For every review, return an object with exactly: review_id, relevant (boolean), themes (array chosen only from ["Wishlist Clutter & Organization", "Size & Stock Availability", "Fit & Fabric Anxiety", "Delivery & Shipping Friction", "Occasion & Styling Inspiration"]), segment (short label), categories (array), barrier (short label or null), sentiment (positive, neutral, or negative), reason (one short sentence). Mark relevant only if the review helps study why users wishlist fashion/beauty items but hesitate to purchase them, or what frictions they encounter at checkout. Do not treat generic coupons, generic delivery complaints, or unrelated app bugs as wishlist relevant unless explicitly linked to wishlist purchase barriers.`;
