...
function getJson(url) {
  return new Promise((resolve, reject) => {
    https.get(url, { headers: { 'User-Agent': 'MyntraResearchDashboard/1.0' } }, response => {
      let body = '';
...
(async () => {
  const all = new Map();
  const seedFiles = [
    `myntra_app_store_global_last_${LIMIT}.json`,
    'myntra_app_store_global_last_2000.json',
    'myntra_app_store_last_1000.json',
    'myntra_app_store_last_500.json'
  ];
  ...
  fs.writeFileSync(path.join(dataDir, `myntra_app_store_global_last_${LIMIT}.json`), JSON.stringify(records, null, 2));
