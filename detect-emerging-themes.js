...
(async () => {
  ...
  const prompt = `You are a product-research analyst for Myntra's Wishlist Conversion Team. Compare these timestamped weekly theme counts: ${evidence}\nReturn JSON with keys: data_sufficiency (insufficient|adequate), emerging_themes (array of objects with theme, direction, evidence, risk_statement, recommended_monitor), new_problem_hypotheses (array), executive_summary (a beautifully written, cohesive paragraph summarizing the week's biggest shifts in wishlist checkout friction), recommended_actions (array of specific, actionable product features/UX experiments that do not involve monetary coupons or discounts), caveat. Only flag an emerging theme when supported by a change across at least two distinct weekly snapshots. Do not predict facts; phrase outputs as risks or hypotheses. If the history is too short or same-day, say insufficient and explain what should be collected next.`;
